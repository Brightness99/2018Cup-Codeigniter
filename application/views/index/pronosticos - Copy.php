				...
				<?php if(count($matchList) > 0){ foreach($matchList as $each_match){  ?>
					<div class="small-item-wrap">
						...
						<div class="time-small-block">
							...
							<div class="roundred leftRound" <?php if($each_match['scored'] != 1 && $diffTotal > 24){ ?> contenteditable="true" <?php } ?> 	onkeyup="$('#home_predection_<?php echo $each_match['match_id']; ?>').val($(this).text())"><?php if(!empty($each_match['user_prediction'])){ echo $each_match['user_prediction']->home_goals; }else{ echo 'x'; }  ?>
							</div>
							<span>X</span>
							<div class="roundred rightRound" <?php if($each_match['scored'] != 1 && $diffTotal > 24){ ?> contenteditable="true" <?php } ?> onkeyup="$('#away_predection_<?php echo $each_match['match_id']; ?>').val($(this).text())"><?php if(!empty($each_match['user_prediction'])){ echo $each_match['user_prediction']->away_goals; }else{ echo 'x'; }  ?>
							</div>
