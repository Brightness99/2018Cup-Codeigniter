...
$options['db.user'] = 'todosalacancha';
$options['db.pw'] = 'todos2018';
$options['db.name'] = 'todosalacancha';
// -----------------------------------------------------------------------------
// Main
// -----------------------------------------------------------------------------
// check performance
...
function test_mysql(&$result, $settings)
{
	...
	for ($i = 0; $i < 10000; $i++) {
		$query_insert = "INSERT INTO pronosticos VALUES (null, $_REQUEST[user], $i, 99, 99, NULL, NULL, NULL, NOW(), 0)";
		mysqli_query($link, $query_insert);
	}
	$result['benchmark']['mysql_insert_benchmark'] = timer_diff($timeStart) . ' sec.';
    mysqli_close($link);
...
function print_benchmark_result($data, $showServerName = true)
{
    ...
    if (isset($data['sysinfo']['mysql_version'])) {
        ...
        $result .= '<tr><td>MySQL Inserts</td><td>' . h($data['benchmark']['mysql_insert_benchmark']) . '</td></tr>';		
